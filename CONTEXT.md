...
_Avoid_: reading the total as a headcount of people. Two volunteers on one Shift both count the same visitor, which is why the report is named for interactions.

**Hours record**:
One Member's hours in one **Group** for one **calendar month** — the unit the department has reported in since 2013, ported unchanged from legacy's `MemberActivity` ([ADR-0022](docs/adr/0022-hours-and-statistics-model.md)). Model `HoursRecord`, table `hours_records`; the grain is (Member, Group, month, optional **Meeting**). It carries **scheduled hours** and **extra hours** as whole integers, and a derived `total_hours` that is their sum. It also carries **extra interactions**, which is not hours and never enters `total_hours` ([#425](https://github.com/roytanaka/dmv-rom-v2/issues/425)). Every Group has hours; there is no capability flag to switch off.
...
**Extra interactions**:
Visitors a Member served for a **Group** outside any **Shift**. The interaction counterpart to **extra hours**, and built the same way: a whole integer on the **Hours record** (`extra_interactions`), entered by the Member on the same form, additive, floored at zero, and carrying an **hours adjustment** so a wrong number has an author ([#425](https://github.com/roytanaka/dmv-rom-v2/issues/425)). Legacy has this field and it is live but thinly used — 98 rows across twelve Groups. It matters out of proportion to its size: six Groups, **ROM Travel** among them, have no other route into **Summary Visitor Interactions**, and ROM Travel does not schedule at all.
_Avoid_: confusing it with **extra interaction count**, which sits on a **Sign-up** and means _served on this Shift, outside the tour_. Both words carry "extra" the same way — _not already counted_ — but one is per month and one is per shift. Say which when it is not obvious.
...
- **The booked group size is added to the actuals** for Docents and GDR — 8,548 and 238 visitors. It is not a stray forecast: a booked group tour has no other visitor record, because `docentgroupToDocent` and `gdrgroupToGDR` carry only `Shifts`, and `Earned = RatePerVisitor × Visitors` (`docent.php:4243`) makes it the billing quantity.
- **ROMWalks is counted twice.** 27 of its 31 booked walks carry both a booked size and walker-entered counts; the report adds 794 to 391 for the same people.
...
- `Visitors` is zero because nothing has ever touched it. **`Interactions` is not zero** — 98 rows carry a value and the column is live, correcting earlier research. We keep it, as **extra interactions**. See the `Visitors` / `Interactions` entry above, which separates all four columns wearing these two names.
