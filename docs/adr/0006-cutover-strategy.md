...
- **Big-bang assigned credentials emailed at cutover.** Temp password emailed to every volunteer. Rejected because plaintext credentials in email are a poor security posture, family-shared inboxes are still common (despite the unique-email rule that comes with cutover), and support load for lost emails would be heavy.
