...
> **Amendment (2026-08-23, [ADR-0022](0022-hours-and-statistics-model.md)).** The structure/content split (§B) gains one **explicit exception**: **hours reporting reads are subtree-wide**. A Group's hours report shows its own hours and its own hours including every descendant, to any depth, and a parent's Chair or Statistician reads those descendant totals without holding a role in the child. Hours are a statistic, not content — a statistic aggregates, a document does not — so a `Private` child's document library stays opaque to that same person. Refusing the exception would make a parent's total stop matching the sum of its children, which is the number the ROM is given. See ADR-0022 §5.
