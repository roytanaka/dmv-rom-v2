...
**2026-05-18** — Production-deploy half of this strategy deferred until cutover prerequisites are closer. Cutover, per [ADR-0006](0006-cutover-strategy.md), depends on (a) feature parity with the legacy app (via parallel running) and (b) the legacy → new DB migration scripts; neither has a PRD yet as of this date. Building `deploy-production.yml` + the `production` GitHub Environment + the production-side `.env` / server prep + the recovery runbook now would mean they sit unused for several months, during which workflow YAML drifts, server-side state can rot, and an untested runbook becomes fiction at exactly the moment it would be reached for. The decision favours staleness avoidance over context-warmth. Staging-deploy (`deploy-staging.yml` + `scripts/deploy.sh`, parameterized by environment) remains the only auto-deploy infrastructure in v1. `scripts/deploy.sh` already has the production branch wired (mysqldump gate, virgin-DB-safe migration detection via #11 / #14 / #15), so the deferred work is incremental when the consuming workstreams are ready. Issue #7 closed; PRD #4 acceptance criteria related to production deploy are deferred along with it.
