...
- **Chrome** — the frame's own words: UI labels, navigation, buttons, validation messages, system emails. Authored by the project, the same for every Volunteer. **Translated.** Source of truth is Laravel `lang/en/*.php` + `lang/fr/*.php`, surfaced to Vue by [`laravel-vue-i18n`](https://github.com/xiCO2k/laravel-vue-i18n), which reads those same files (no second message store on the JS side). Interpolation uses Laravel's `:placeholder` syntax. _Amended by [ADR-0025](0025-help-centre.md) (2026-09-13): help articles are chrome too, but live as Markdown files in their own tree rather than in `lang/`. Same translation workflow: machine-translated French shipped with the English._
...
- **Exception — institutional/legal strings use ROM's official French verbatim, never machine translation.** Three keys: the **land acknowledgement**, the **inclusion statement**, and the **department name**. They live in a dedicated `lang/{en,fr}/institutional.php` that the machine-translation step **skips by filename** — a structural wall, not a per-string annotation. The exclusion list is `config('translation.machine_translation_excludes')` (`config/translation.php`); the MT step consults it, and a Seam B Pest test pins both the exclusion and the official French verbatim. Official French for the land acknowledgement and inclusion statement is now hand-authored verbatim; the department name **stays English in both locales** ("ROM Department of Museum Volunteers") until ROM provides approved French.
