...
> **Amendment (2026-09-22, [ADR-0026](0026-gallery-interpreters-self-serve-shifts.md) §7).** The column is a **decimal**, not an integer. Gallery Interpreters is the second non-unit Group: legacy credits one whole shift per 45-minute unit with no factor, so GI's multiplier is **4/3** (stored 1.333), which makes _recalculate this month_ reproduce the legacy number exactly through the existing summed-minutes path. Walker's 2 and every 1 are unchanged.
