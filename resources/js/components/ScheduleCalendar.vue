...
import { buildMonthGrid, monthsInRange, type DayGroup, type MonthCell } from '@/scheduling/agenda';
...
const props = defineProps<{ agenda: DayGroup<ShiftAgendaItem>[]; startsOn: string; endsOn: string }>();
...
const openSheet = (cell: MonthCell<ShiftAgendaItem>) => {
    if (cell.date && cell.shifts.length) openDay.value = cell;
};
            ...
            <template v-for="(week, w) in grid.weeks" :key="w">
                <template v-for="(cell, d) in week" :key="`${w}-${d}`">
                    ...
                    >
                        ...
                        <span class="bg-secondary text-secondary-foreground mt-auto self-start rounded-full px-1.5 py-0.5 text-xs">
