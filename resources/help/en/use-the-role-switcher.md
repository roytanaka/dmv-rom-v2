# Use the Role-switcher

Become a Persona to see the app as they do. This tool runs on staging and local only.

## Switch to a Persona

1. Find the Role-switcher button in the bottom-right corner.
2. Select it to open the list of Personas.
3. Select the Persona you want to become.

The app reloads as that Persona. You now see every page with their roles and standing.

> **Note:** You need the Support-operator marker to see the button. It never appears in production.

## What next

Read _Return to yourself after impersonating_ to get back to your own account.
